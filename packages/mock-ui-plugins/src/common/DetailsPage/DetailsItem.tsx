...
import { K8sResourceKind, referenceFor } from "../utils";
...
export const DetailsItem: FC<DetailsItemProps> = ({
  ...
  const hide = hideEmpty && _.isEmpty(_.get(obj, path as any));
  const popoverContent: string = description ?? "No description available";
  const value: ReactNode = children || _.get(obj, path as any, defaultValue);
                ...
                bodyContent={
                  <a href="#">
                    <div className="co-pre-line">
                      {popoverContent}, should be external link
                    </div>
                  </a>
                }
                {...(path && {
